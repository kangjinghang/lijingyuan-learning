...
public class Slf4jReporterExample {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(Slf4jReporterExample.class);
    ...
    private static final Slf4jReporter SLF4J_REPORTER = Slf4jReporter.forRegistry(REGISTRY)
            .outputTo(LoggerFactory.getLogger("com.lijingyuan.metrics"))
            .convertRatesTo(TimeUnit.SECONDS)
            .convertDurationsTo(TimeUnit.SECONDS)
            .build();
...
//        CONSOLE_REPORTER.start(10, TimeUnit.SECONDS);
        SLF4J_REPORTER.start(1, TimeUnit.SECONDS);
